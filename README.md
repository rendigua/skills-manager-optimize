...
- **Origin metadata** — Every installed skill carries an `origin.json` file so source, confidence, and update eligibility survive across machines and rebuilds.
...
- Agent-generated skills should also write `origin.json`, defaulting to `manual + custom-no-source` unless a real upstream is explicitly bound.
- The source resolver prefers `skills.sh` first. `SkillsMP` only participates when `skillsmp_api_key` is configured in settings or `SKILLSMP_API_KEY` is present in the environment.
- `SkillsMP` results are treated as distribution evidence, not as a replacement for GitHub upstream truth.
- The `My Skills` page also exposes a bulk backfill action that only auto-applies unambiguous matches and leaves ambiguous items for review.
